                            ...
                            <form action="#" method="POST" name="login">
                                ...
                                <div class="form-group row">
                                    <label for="password" class="col-md-4 col-form-label text-md-right">New
                                        Password</label>
                                    <div class="col-md-6">
                                        <input type="password" id="password" class="form-control" name="password" pattern="^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[$@$!%*?&])[A-Za-z\d$@$!%*?&]{6,20}" 
...
<?php
    if(isset($_POST["reset"])){
        ...
        if($email){
            $new_pass = $psw;
            mysqli_query($connect, "UPDATE user SET u_pass='$new_pass' WHERE email='$email'");
            ?>
<script>
